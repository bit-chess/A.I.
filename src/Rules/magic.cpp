...
uint16_t magicindexrook(uint64_t occupied, int square, uint64_t magic){
    ...
    uint64_t inter = (blockers * magic);
    uint64_t index = (inter) >> (64 - rookshift[square]);
...
int testmagicrook(int square, uint64_t magic) {
    ...
    int count = 0;
    do {      
      uint64_t attack = moverook[square];
      uint16_t index = magicindexrook(n, square, magic);
